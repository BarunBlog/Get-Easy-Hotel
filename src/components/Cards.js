...
function Cards() {
    const path = '/hotel'
    const firstHalfOfHotels = HotelData.filter( (hotel, i) =>
        i < HotelData.length / 2
    );
    const secondHalfOfHotels = HotelData.filter( (hotel, i) =>
        i >= HotelData.length / 2
    );
    return (
        <div className='cards'>
